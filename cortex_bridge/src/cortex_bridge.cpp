...
//	ros::NodeHandle param_nh("~");
    nh.getParam("local_ip", myIP);
	if ( !nh.getParam("local_ip", myIP) )
		ROS_ERROR ("Could not find IP of this machine in launch file.");
	if ( !nh.getParam("cortex_ip", cortexIP) )
	...
	if ( InitializeCortexConnection ( const_cast<char*>(myIP.c_str()), const_cast<char*>(cortexIP.c_str() )) != 0 )
	{
